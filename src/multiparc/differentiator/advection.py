...
class Advection(nn.Module):
    ...
    def forward(self, state_variable, velocity_field):
        ...
        Returns:
            advect (torch.Tensor): Advection term of shape [N, C, H, W]
        """
        dy, dx = self.cdiff(state_variable)  # Each of shape [N, C, H, W]
        # Advection = dx * vx + dy * vy
        advect = dx * velocity_field[:, 0:1, :, :] + dy * velocity_field[:, 1:2, :, :]
        return advect
...
class AdvectionUpwind(nn.Module):
    ...
    def forward(self, state_variable, velocity_field):
        ...
        Returns:
            advect (torch.Tensor): Advection term of shape [N, C, H, W]
        """
        dy_l, dx_l = self.ldiff(state_variable)  # Each of shape [N, C, H, W]
